...
export function useChannelMessages() {
  ...
  const fetchMessages = useCallback(async (channelId: string, embedsOnly = false, preserveSelection = false) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      if (!preserveSelection) {
        ...
        setSelectedMessageId((prev) => (prev && result.some((m) => m.id === prev) ? prev : null));
      }
    } catch (err) {
  ...
  const sendMessage = useCallback(
    async (channelId: string, payload: { content?: string; embeds?: MessageEmbed[] }) => {
      setLoading(true);
      setError(null);
